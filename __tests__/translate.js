...
test('translate (pong)', async () => {
  const input = await fs.readFile(path.join(__dirname, '__fixtures__', 'input', 'pong'), 'utf-8');
  const output = await fs.readFile(path.join(__dirname, '__fixtures__', 'output', 'pong'), 'utf-8');
